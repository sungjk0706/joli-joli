...
import { MessageSquare, Trash, Trash2, ShieldOff, Send, Bell } from 'lucide-react';

const AdminChat = ({ showAlert }) => {
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          <h2 className="text-2xl sm:text-3xl admin-title flex items-center gap-2">
            <MessageSquare size={28} className="text-zinc-800" />
            실시간 채팅 관리
          </h2>
          <p className="admin-text-secondary text-sm">라이브 방송 중인 채팅 메시지를 모니터링하고 부적절한 내용을 삭제합니다.</p>
        </div>
        <button
          onClick={handleClearChat}
          className="px-5 py-2.5 bg-red-500 text-white rounded-xl font-black flex items-center gap-2 hover:bg-red-600 transition-all shadow-lg shadow-red-500/20 active:scale-95"
        >
          <Trash size={18} />
          <span>채팅창 비우기</span>
        </button>
      ...
      <div className="bg-zinc-900 rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl flex flex-col h-[calc(100vh-420px)] min-h-[400px]">
